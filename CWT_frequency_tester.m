%%% CWTPlot_APHT_freq_tester 

% this script tests with fake imported data if the frequency y axis for the
% cwt plotter is accurate 
...
% g = @(t)(sin(120*pi*t)) ; 
    ...
    gd = 10000; % Sampling frequency (samples per second) 
    dt = 1/gd; % seconds per sample 
    StopTime = 10000 ; % seconds 
    t = (0:dt:StopTime)'; % seconds 
%     t = (0:dt:1205)';
    F = 40; % Sine wave frequency (hertz) 
    donut = sin(2*pi*F*t);
    plot(t,donut) %%For one cycle get time period
    T = 1/F ; % time step for one time period 
    tt = 0:0.01:602899.99 ;
%     tt = 0: ;
    d = sin(2*pi*F*tt) ;
    FD = plot(tt,d) ;
...
if decision == 1
    ...
    FD = input('What channel is fake data?:');
    stim = input('What channel is stim data?:') ;
    RS=set_channels(RS);LS=set_channels(LS);RH=set_channels(RH);LH=set_channels(LH);stim=set_channels(5);FD=set_channels(6) ; 
end 
if decision == 0  
    % 5/5
...
names={'RSdata','LSdata','RHdata','LHdata', 'stimdata', 'LHRSbipolardata', 'LHRHbipolardata', 'FD'};
%% plot raw data
figure
